...
public class BasicAndroidKeyStoreFragment extends Fragment{
    ...
    FingerprintManagerCompat.AuthenticationCallback authenticationCallback = null;
...
//                    .build();
            spec = new KeyGenParameterSpec.Builder(mAlias, KeyProperties.PURPOSE_SIGN | KeyProperties.PURPOSE_VERIFY)
                    .setCertificateSubject(new X500Principal("CN=" + mAlias))
                    .setDigests(KeyProperties.DIGEST_SHA256)
    ...
    public void stopListening(CancellationSignal cancellationSignal) {
    ...
    private void signDataAction() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            ...
            try {
                ...
                Log.i(TAG, "Key Permanently Invalidated");
            } catch (KeyStoreException e) {
                Log.w(TAG, "KeyStore not Initialized", e);
            } catch (UnrecoverableEntryException e) {
                Log.w(TAG, "KeyPair not recovered", e);
